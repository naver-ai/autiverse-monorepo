    ...
    async def start_revision_conversation(self):
        ...
        while self.revision_count < self.max_revisions:
            ...
            if user_response.lower() in ['아니', '아니요', 'no', 'n']:
                ...
                if self.revision_count == self.max_revisions:
                    print("도도: 아앗;; 이제 마지막 기회야! 지금 틀린 부분이 있다면 다 말해줘~ 😅")
                else:
                    print("도도: 아앗;; 어디가 어떻게 틀렸어? 😅")
...
if __name__ == "__main__":
    ...
    example_input = {
        ...
        "panel2": {"content": None, "missing_content": "왜 혼났는지 알 수 없음"},
        "panel3": {"content": "갑자기 선생님이 나타나서 나를 혼냈다", "missing_content": None},
        "panel4": {"content": None, "missing_content": "감정 없음"}
    }
